...
export async function membership(userId, tripId) {
...
export async function authorize(userId, tripId, permission) {
  const member = await membership(userId, tripId);
...
export async function assertResourceInTrip(table, resourceId, tripId) {
  const resource = await db.prepare(`SELECT * FROM ${table} WHERE id=? AND trip_id=?`).get(resourceId, tripId);
