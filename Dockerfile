...
HEALTHCHECK --interval=30s --timeout=5s --start-period=10s --retries=3 \
  CMD ["deno", "eval", "--allow-net=127.0.0.1:4173", "const response = await fetch('http://127.0.0.1:4173/api/health'); if (!response.ok) Deno.exit(1)"]

CMD ["deno", "run", "--cached-only", "--allow-net=0.0.0.0:4173", "--allow-read=/app", "--allow-write=/app/data", "--allow-env=PORT,TABI_DATABASE_PATH,TABI_SECURE_COOKIE", "server.js"]
